"""
Convert a portrait photo into a CLEAN, monochrome ASCII-art SVG (subject
isolated on a dark background) that "types" itself in like a terminal, then
holds.
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "clash-ascii.svg")
...
# --- Bleach-tinged terminal palette: near-black bg, bone-white ink, a single
#     reiatsu-blue accent used only for the cursor / prompt glyphs.
BG = "#0a0a0c"
BG2 = "#101014"
FRAME = "#2a2d34"
TITLE_TEXT = "#7d8590"
INK = "#e8e6e1"       # single ascii color -- bone/off-white
CURSOR = "#5ec8d8"    # reiatsu-blue accent
PROMPT = "#5ec8d8"
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">clashlex@github: ~$ ./bankai --portrait</text>')
